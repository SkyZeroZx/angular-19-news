...
export function createSwipeSubscription({
	...
	const touchStarts$ = fromEvent<TouchEvent>(domElement, 'touchstart', eventListenerOptions).pipe(
		map(getTouchCoordinates)
	);
	const touchMoves$ = fromEvent<TouchEvent>(domElement, 'touchmove', eventListenerOptions).pipe(
		map(getTouchCoordinates)
	);
	const touchEnds$ = fromEvent<TouchEvent>(domElement, 'touchend', eventListenerOptions).pipe(
		map(getTouchCoordinates)
	);
	const touchCancels$ = fromEvent<TouchEvent>(domElement, 'touchcancel', eventListenerOptions);
						...
						race(
							touchEnds$.pipe(
								...
								tap((coordinates: SwipeCoordinates) => {
									...
									const swipeEvent = getSwipeEvent(touchStartEvent, coordinates);
